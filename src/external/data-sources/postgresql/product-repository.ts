...
export class ProductRepository implements ProductDataSource {
  ...
  constructor(manager: EntityManager) {
    ...
    async update(id: number, product: ProductEntity): Promise<ProductEntity> {
        const query = { id: (id)};
        await this.repository.update(query, {product});
        return product;
    }
    
    async getOne(id: number): Promise<ProductEntity> {
        const query = { id: (id)};
        const produto = await this.repository.findOneBy(query) as ProductEntity;
        if (!produto) {
    ...
    async getAll(): Promise<ProductEntity[]> {
        if((await this.repository.find()).length >0) {
            await this.repository.save(this.produtos);
        }
        return await this.repository.find() as ProductEntity[];
    }
