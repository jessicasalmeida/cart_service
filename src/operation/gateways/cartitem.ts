...
export class CartItemGateway {
    ...
    async createcart(cart: NewCartItemDTO): Promise<CartItemDTO | null> {
        const cartEntity: CartItemEntity = new CartItemEntity(
            ...
            new CartEntity(Number(cart.cart.id), new UserEntity(Number(cart.cart.user.id), cart.cart.user.cpf, cart.cart.user.name, cart.cart.user.email, cart.cart.user.cep, cart.cart.user.telefone), cart.cart.totalValue, cart.cart.status, cart.cart.payment, cart.cart.estimatedTime));
